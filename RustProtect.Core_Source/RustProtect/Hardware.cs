namespace RustProtect
{
    ...
    public class Hardware
    {
        ...
        private static string string_0 = DateTime.Now.ToString("yyyyMMddHH");
        ...
        public Hardware()
        {
            using (BinaryWriter writer = new BinaryWriter(new MemoryStream()))
            {
                ...
                {
                    Debug.Log("Calculating Hardware ID...");
                }
                ...
                if (strArray.Length != 0)
                {
                    ...
                    if (length != 0)
                    {
                        ...
                        if (ProtectLoader.Debug)
                        {
                            Debug.Log("Hardware BIOS Received.");
                        }
                        ...
                        if (Class1.CallWindowProcW(buffer2, IntPtr.Zero, 0, buffer5, ptr) != IntPtr.Zero)
                        {
                            if (ProtectLoader.Debug)
                            {
                                Debug.Log("Hardware CPU Received.");
                            }
                            ...
                            if (ProtectLoader.Debug)
                            {
                                Debug.Log("Hardware GPU Received.");
                            }
                            ...
                            if (ProtectLoader.Debug)
                            {
                                Debug.Log("Hardware ID Succesfully Generated.");
                            }
                        }
                    }
                }
        ...
        private PhysicalDisk method_4(string string_1)
        {
            ...
            SafeFileHandle handle = Class1.CreateFileW(@"\\.\" + Path.GetPathRoot(string_1).TrimEnd(trimChars), 0xc0000000, 3, IntPtr.Zero, 3, 0x80, IntPtr.Zero);
            try
            {
                uint num = 0;
        ...
        public string SHA256String
        {
            get
            {
                if ((byte_2 != null) && (byte_2.Length >= 0x10))
                {
                    return BitConverter.ToString(byte_2, 0).Replace("-", "").ToLower();
                }
                return "";
            }
        }
        ...
        public string SHA512String
        {
            get
            {
                if ((byte_3 != null) && (byte_3.Length >= 0x10))
                {
                    return BitConverter.ToString(byte_3, 0).Replace("-", "").ToLower();
                }
                return "";
            }
        }
        ...
        public string String_0
        {
            get
            {
                if ((byte_0 != null) && (byte_0.Length >= 0x10))
                {
                    return BitConverter.ToString(byte_0, 0).Replace("-", "").ToLower();
                }
                return "";
            }
        }
        ...
        public string String_1
        {
            get
            {
                if ((byte_1 != null) && (byte_1.Length >= 0x10))
                {
                    return BitConverter.ToString(byte_1, 0).Replace("-", "").ToLower();
                }
                return "";
            }
        }
        ...
        public struct RawSMBIOSData
        {
            ...
            public Hardware.GStruct1 CacheInfo;
        }
